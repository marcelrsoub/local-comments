...
function saveComments() {
    try {
        ...
        vscode.window.showInformationMessage(`Comments saved to ${commentFilePath}`);
    } catch (err) {
...
function openCommentInput() {
    ...
    vscode.window.showInputBox({
        ...
        if (comment !== undefined) {
            ...
            updateDecorations();
            saveComments();
            vscode.window.showInformationMessage(`Comment saved for line ${lineNumber} in ${fileName}`);
        }
    });
}
